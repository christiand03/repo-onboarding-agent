...
## 1. Project Overview
- **Description:** A comprehensive AI-powered agent designed to automate the onboarding and documentation process for Git repositories. It leverages Large Language Models (LLMs) to perform in-depth code analysis, extract project metadata, and generate structured documentation, complete with architecture insights and detailed code explanations. It features a Streamlit-based frontend for user interaction and a MongoDB backend for managing users and chat history.
- **Key Features:**
  - LLM-powered code analysis (HelperLLM, MainLLM).
  - Automated repository cloning and file parsing.
  - Generation of Abstract Syntax Trees (AST) and call graphs.
  - Extraction of project metadata (dependencies, tech stack, etc.).
  - Interactive Streamlit frontend for user management and chat-based documentation generation.
- **Tech Stack:** Python, Streamlit, LangChain, Google Gemini, OpenAI, Ollama, MongoDB, GitPython, Pydantic, NetworkX, Matplotlib, nbformat, Cryptography.

*   **Repository Structure:**
    ```mermaid
    graph TD
        root
        root --> A[root_level_files<br/>.env.example<br/>.gitignore<br/>analysis_output.json<br/>output.json<br/>output.toon<br/>readme.md<br/>requirements.txt<br/>test.json]
        root --> B[SystemPrompts<br/>SystemPromptClassHelperLLM.txt<br/>SystemPromptFunctionHelperLLM.txt<br/>SystemPromptHelperLLM.txt<br/>SystemPromptMainLLM.txt<br/>SystemPromptMainLLMToon.txt<br/>SystemPromptNotebookLLM.txt]
        root --> C[backend<br/>AST_Schema.py<br/>File_Dependency.py<br/>HelperLLM.py<br/>MainLLM.py<br/>__init__.py<br/>basic_info.py<br/>callgraph.py<br/>converter.py<br/>getRepo.py<br/>main.py<br/>relationship_analyzer.py<br/>scads_key_test.py]
        root --> D[database<br/>db.py]
        root --> E[frontend<br/>__init__.py<br/>frontend.py]
        E --> E1[.streamlit<br/>config.toml]
        E --> E2[gifs<br/>4j.gif]
        root --> F[notizen<br/>Report Agenda.txt<br/>Zwischenpraesentation Agenda.txt<br/>doc_bestandteile.md<br/>notizen.md<br/>paul_notizen.md<br/>praesentation_notizen.md<br/>technische_notizen.md]
        F --> F1[grafiken]
        F1 --> F1a[1<br/>File_Dependency_Graph_Repo.dot<br/>global_callgraph.png<br/>global_graph.png<br/>global_graph_2.png<br/>repo.dot]
        F1 --> F1b[2<br/>FDG_repo.dot<br/>fdg_graph.png<br/>fdg_graph_2.png<br/>filtered_callgraph_flask.png<br/>filtered_callgraph_repo-agent.png<br/>filtered_callgraph_repo-agent_3.png<br/>filtered_repo_callgraph_flask.dot<br/>filtered_repo_callgraph_repo-agent-3.dot<br/>filtered_repo_callgraph_repo-agent.dot<br/>global_callgraph.png<br/>graph_flask.md<br/>repo.dot]
        F1 --> F1c[Flask-Repo<br/>(65 files)]
        F1 --> F1d[Repo-onboarding<br/>(15 files)]
        root --> G[result<br/>(28 files)]
        root --> H[schemas<br/>types.py]
        root --> I[statistics<br/>(5 files)]
    ```
...
The Repo Onboarding Agent is primarily designed for automating the documentation of software repositories.

**Key Use Cases:**
1.  **Automated Repository Documentation:** Users provide a Git repository URL, and the agent automatically clones, analyzes, and generates a detailed Markdown documentation report. This includes high-level overviews, architectural insights, and in-depth code analysis for functions and classes.
2.  **LLM-Powered Code Analysis:** Specialized Helper LLMs analyze individual functions and classes to provide detailed descriptions, parameter breakdowns, return values, and usage contexts, significantly reducing manual documentation effort.
3.  **Architectural Visualization:** The system dynamically builds and integrates architectural insights like Abstract Syntax Trees (AST) schemas and call graphs, aiding in understanding complex code structures and dependencies.
4.  **Interactive Frontend for Documentation Generation:** A Streamlit-based user interface (`frontend/frontend.py`) allows users to trigger documentation workflows, manage API keys for various LLMs, and view generated reports in a chat-like interaction model.
5.  **User and Chat Management:** A MongoDB backend (`database/db.py`) enables user authentication, secure storage of API keys, and management of documentation generation history and user feedback on generated reports. This allows for personalized experiences and continuous improvement.
6.  **Notebook Documentation:** The agent can process Jupyter notebooks, converting them into a structured XML format and generating reports, including handling embedded images.

**Primary Commands (Inferred):**
*   **Start the Frontend:** To run the interactive web application, a typical command would be:
    ```bash
    streamlit run frontend/frontend.py
    ```
*   **Internal Workflow Execution:** The core documentation generation is triggered via the frontend, which internally calls `backend.main.main_workflow` or `backend.main.notebook_workflow` after a repository URL is provided.
*   **Database Interactions:** User registration, login, API key management, and chat history storage are handled internally by `database/db.py` functions, typically invoked by the frontend.

## 4. Architecture
The Mermaid Syntax to visualize Graphs is not set up yet and will be added
but if there is mermaid syntax in your input json display it here
No global Mermaid architecture diagram was provided in the input data.
...
*   **Signature:** `def build_file_dependency_graph(filename: str, tree, repo_root: str)`
...
*   **Instantiation:** This class is not explicitly instantiated by other functions or methods according to the provided context.
