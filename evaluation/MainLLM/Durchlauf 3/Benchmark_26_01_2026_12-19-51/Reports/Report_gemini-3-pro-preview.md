...
## 1. Project Overview
*   **Description:** Repo Onboarding Agent 🚀 is an automated code analysis and documentation tool designed to facilitate developer onboarding. It functions by cloning a Git repository, analyzing its structure (AST), and using a dual-LLM architecture (Helper LLM and Main LLM) to generate comprehensive documentation. The system includes a Streamlit-based frontend for interactive visualization of dependency graphs, call graphs, and chat-based querying of the codebase. It also supports the conversion and analysis of Jupyter Notebooks.
*   **Key Features:**
    -   **Automated Repository Analysis:** Clones repositories and performs static analysis to generate Abstract Syntax Trees (AST) and identify file dependencies.
    -   **Dual-LLM Architecture:** Utilizes a "Helper LLM" for granular function/class analysis and a "Main LLM" for synthesizing final reports.
    -   **Interactive Frontend:** A Streamlit dashboard for managing API keys, viewing analysis reports, and visualizing project structure via Mermaid/Graphviz.
    -   **Visualization:** Generates file dependency graphs and call graphs to visualize the architecture and flow of the target project.
    -   **Jupyter Notebook Support:** Converts and analyzes `.ipynb` files, extracting code and images for LLM processing.
*   **Tech Stack:** Python, Streamlit, MongoDB (pymongo), LangChain (Google GenAI, Ollama, OpenAI), NetworkX, GitPython, Pydantic.
...
*   **Repository Structure:**
    ...
    root --> frontend
    frontend --> frontend_files["__init__.py<br/>frontend.py"]
    frontend --> .streamlit
    .streamlit --> .streamlit_files["config.toml"]
    frontend --> gifs
    gifs --> gifs_files["4j.gif"]
    root --> notizen
    notizen --> notizen_files["Report Agenda.txt<br/>Zwischenpraesentation Agenda.txt<br/>doc_bestandteile.md<br/>notizen.md<br/>paul_notizen.md<br/>praesentation_notizen.md<br/>technische_notizen.md"]
    notizen --> grafiken
    root --> result
    root --> schemas
...
2.  Ensure you have a MongoDB instance running (local or cloud) as required by `database/db.py`.
3.  Copy `.env.example` to `.env` and fill in the necessary API keys (OpenAI, Google Gemini, etc.) and database connection strings.
4.  Install the dependencies using the command above.
...
*   **Analyze a New Repository:** Users can input a GitHub URL in the frontend. The system runs `backend.main.main_workflow` to clone, analyze (AST/Dependency), and document the repository.
*   **Chat with Codebase:** Users can ask questions about the analyzed repository. The system uses stored analysis and LLMs to answer questions or generate code snippets.
*   **Visualize Architecture:** The frontend renders Mermaid diagrams and Graphviz plots generated by `backend.File_Dependency` and `backend.callgraph` to show how files and functions interact.
*   **Notebook Analysis:** The `notebook_workflow` allows for specific analysis of Jupyter Notebooks found within a repository.

## 4. Architecture
*No Mermaid syntax was provided in the input JSON for this section.*
...
*   **Methods:**
    ...
    *   **`visit_ClassDef`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `ast.get_docstring`, `ast.get_source_segment`, and `self.generic_visit`. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.ClassDef` node is encountered.
    *   **`visit_FunctionDef`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `ast.get_docstring`, `ast.get_source_segment`, and `self.generic_visit`. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.FunctionDef` node is encountered, and also by `visit_AsyncFunctionDef`.
    *   **`visit_AsyncFunctionDef`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `self.visit_FunctionDef`. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.AsyncFunctionDef` node is encountered.
...
*   **Methods:**
    *   **`merge_relationship_data`**
        ...
        *   **Usage:** This method primarily uses dictionary 'get' methods and iterates over data structures. It does not call other distinct functions or classes. Called by `backend.main.main_workflow`.
    *   **`analyze_repository`**
        ...
        *   **Usage:** Calls 'os.path.commonpath', 'os.path.isfile', 'os.path.dirname', 'ast.parse', and instantiates 'ASTVisitor'. Called by `backend.main.main_workflow`.
...
*   **Usage:** This function calls no other functions. Called by `backend.AST_Schema.ASTVisitor.__init__`.
...
*   **Methods:**
    *   **`_resolve_module_name`**
        ...
        *   **Usage:** Calls get_all_temp_files to get all files in the repository, Path for path manipulation, iskeyword to check for Python keywords, and internally defines and calls module_file_exists and init_exports_symbol to verify module existence and symbol exports. It also uses literal_eval, parse, and walk from the ast module within init_exports_symbol. Called by visit_ImportFrom when processing relative import statements.
    *   **`visit_Import`**
        ...
        *   *Returns:*
        *   **Usage:** Calls self.generic_visit to continue the AST traversal. Called by the AST NodeVisitor when encountering Import nodes, and explicitly by visit_ImportFrom to record dependencies.
    *   **`visit_ImportFrom`**
        ...
        *   *Returns:*
        *   **Usage:** Calls self.visit_Import to record the dependency and self._resolve_module_name to handle relative imports. It also calls self.generic_visit to continue the AST traversal. Called by the AST NodeVisitor framework when traversing the AST and encountering an ImportFrom node.
...
*   **Usage:** Calls `backend.File_Dependency.FileDependencyGraph`. Called by `backend.File_Dependency.build_repository_graph`.
...
*   **Usage:** Calls `backend.File_Dependency.build_file_dependency_graph`. Called by `backend.File_Dependency`.
...
*   **Usage:** This function calls no other functions. Called by `backend.File_Dependency.FileDependencyGraph._resolve_module_name`.
...
*   **Methods:**
    ...
    *   **`generate_for_functions`**
        ...
        *   **Usage:** This method does not explicitly call other methods, classes, or functions based on the provided context. Called by `backend.main.main_workflow`.
...
*   **Parameters:**
*   **Returns:**
*   **Usage:** Calls `backend.HelperLLM.LLMHelper`, `schemas.types.ClassAnalysisInput`, and `schemas.types.ClassContextInput`. Called by `backend.HelperLLM`.
...
*   **Dependencies:** `logging`, `os`, `langchain_google_genai.ChatGoogleGenerativeAI`, `langchain_ollama.ChatOllama`, `langchain_openai.ChatOpenAI`, `langchain.messages.HumanMessage`, `langchain.messages.SystemMessage`
*   **Constructor:**
...
*   **Methods:**
    *   **`call_llm`**
        ...
        *   **Usage:** Calls SystemMessage, HumanMessage, logging.info, self.llm.invoke, and logging.error. Called by `backend.main.main_workflow`, `backend.main.notebook_workflow`.
    *   **`stream_llm`**
        ...
        *   **Usage:** Calls SystemMessage, HumanMessage, logging.info, self.llm.stream, and logging.error.
...
*   **Methods:**
    *   **`_clean_content`**
        ...
        *   **Usage:** This method does not call any other functions or methods. Called by `_parse_readme`, `_parse_toml`, and `_parse_requirements`.
    *   **`_finde_datei`**
        ...
        *   **Usage:** This method does not call any other functions or methods. Called by `extrahiere_info`.
    *   **`_extrahiere_sektion_aus_markdown`**
        ...
        *   **Usage:** Calls re.escape, re.compile, and pattern.search for regular expression operations. Called by `_parse_readme`.
    *   **`_parse_readme`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `_clean_content`, `re.search`, and `_extrahiere_sektion_aus_markdown`. Called by `extrahiere_info`.
    *   **`_parse_toml`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `_clean_content`, `tomllib.loads`, and `print`. Called by `extrahiere_info`.
    *   **`_parse_requirements`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `_clean_content`. Called by `extrahiere_info`.
    *   **`extrahiere_info`**
        ...
        *   **Usage:** Calls `_finde_datei`, `_parse_toml`, `_parse_requirements`, `_parse_readme`, `os.path.basename`, and `repo_url.removesuffix`. Called by `backend.main.main_workflow`, `backend.main.notebook_workflow`.
...
*   **Methods:**
    *   **`_recursive_call`**
        ...
        *   **Usage:** This method does not explicitly call other functions or methods within its own source code. Called by `visit_Call`.
    ...
    *   **`_make_full_name`**
        ...
        *   **Usage:** This method does not explicitly call other functions or methods within its own source code. Called by `visit_FunctionDef`.
    *   **`_current_caller`**
    ...
    *   **`visit_Import`**
        ...
        *   *Parameters:*
            - **node** (`ast.Import`): The AST node representing an import statement.
        *   *Returns:*
        *   **Usage:** Calls `self.generic_visit`. Called by the `ast.NodeVisitor` framework when an `ast.Import` node is visited.
    *   **`visit_ImportFrom`**
        *   *Signature:* `def visit_ImportFrom(self, node: ast.ImportFrom)`
        ...
        *   *Parameters:*
            - **node** (`ast.ImportFrom`): The AST node representing a from ... import ... statement.
        *   *Returns:*
        *   **Usage:** This method does not explicitly call other functions or methods within its own source code. Called by the `ast.NodeVisitor` framework when an `ast.ImportFrom` node is visited.
    *   **`visit_ClassDef`**
        *   *Signature:* `def visit_ClassDef(self, node: ast.ClassDef)`
        ...
        *   *Parameters:*
            - **node** (`ast.ClassDef`): The AST node representing a class definition.
        *   *Returns:*
        *   **Usage:** Calls `self.generic_visit`. Called by the `ast.NodeVisitor` framework when an `ast.ClassDef` node is visited.
    *   **`visit_FunctionDef`**
        *   *Signature:* `def visit_FunctionDef(self, node: ast.FunctionDef)`
        ...
        *   *Parameters:*
            - **node** (`ast.FunctionDef`): The AST node representing a function definition.
        *   *Returns:*
        *   **Usage:** Calls `self._make_full_name`, `self.generic_visit`, `self.graph.add_node`, and `self.function_set.add`. Called by the `ast.NodeVisitor` framework when an `ast.FunctionDef` node is visited, and by `visit_AsyncFunctionDef`.
    *   **`visit_AsyncFunctionDef`**
        *   *Signature:* `def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef)`
        ...
        *   *Parameters:*
            - **node** (`ast.AsyncFunctionDef`): The AST node representing an asynchronous function definition.
        *   *Returns:*
        *   **Usage:** Calls `self.visit_FunctionDef`. Called by the `ast.NodeVisitor` framework when an `ast.AsyncFunctionDef` node is visited.
    *   **`visit_Call`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `self._current_caller`, `self._recursive_call`, `self._resolve_all_callee_names`, and `self.generic_visit`. It also interacts with `self.edges`. Called by the `ast.NodeVisitor` framework when an `ast.Call` node is visited.
    *   **`visit_If`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `self.generic_visit`. Called by the `ast.NodeVisitor` framework when an `ast.If` node is visited.
...
*   **Returns:**
*   **Usage:** This function calls no other functions. Called by `backend.callgraph`.
...
*   **Usage:** Calls `backend.callgraph.CallGraph`. Called by `backend.callgraph`.
...
*   **Usage:** This function calls no other functions. Called by `backend.converter.convert_notebook_to_xml`.
...
*   **Usage:** Calls `backend.converter.process_image`. Called by `backend.converter.convert_notebook_to_xml`.
...
*   **Usage:** This function calls no other functions. Called by `backend.converter.extract_output_content`.
...
*   **Usage:** Calls `backend.converter.extract_output_content` and `backend.converter.wrap_cdata`. Called by `backend.converter.process_repo_notebooks`.
...
*   **Usage:** Calls `backend.converter.convert_notebook_to_xml`. Called by `backend.main.notebook_workflow`.
...
*   **Methods:**
    *   **`blob`**
        ...
        *   **Usage:** This method does not explicitly call other methods or functions.
    *   **`content`**
...
*   **Methods:**
    *   **`get_all_files`**
        ...
        *   **Usage:** Calls `backend.getRepo.RepoFile` to create `RepoFile` instances.
    *   **`close`**
...
*   **Returns:**
*   **Usage:** This function calls no other functions. Called by `backend.main.main_workflow`.
...
*   **Usage:** Calls `backend.AST_Schema.ASTAnalyzer`, `backend.AST_Schema.ASTAnalyzer.analyze_repository`, `backend.AST_Schema.ASTAnalyzer.merge_relationship_data`, `backend.HelperLLM.LLMHelper`, `backend.HelperLLM.LLMHelper.generate_for_classes`, `backend.HelperLLM.LLMHelper.generate_for_functions`, `backend.MainLLM.MainLLM`, `backend.MainLLM.MainLLM.call_llm`, `backend.basic_info.ProjektInfoExtractor`, `backend.basic_info.ProjektInfoExtractor.extrahiere_info`, `backend.getRepo.GitRepository`, `backend.main.calculate_net_time`, `backend.main.create_savings_chart`, `backend.main.update_status`, `backend.relationship_analyzer.ProjectAnalyzer`, `backend.relationship_analyzer.ProjectAnalyzer.analyze`, `backend.relationship_analyzer.ProjectAnalyzer.get_raw_relationships`, `schemas.types.ClassAnalysisInput`, `schemas.types.ClassContextInput`, `schemas.types.FunctionAnalysisInput`, `schemas.types.FunctionContextInput`, and `schemas.types.MethodContextInput`. Called by `frontend.frontend`.
...
*   **Usage:** This function calls no other functions. Called by `backend.main.main_workflow`, `backend.main.notebook_workflow`.
...
*   **Usage:** Calls `backend.MainLLM.MainLLM`, `backend.MainLLM.MainLLM.call_llm`, `backend.basic_info.ProjektInfoExtractor`, `backend.basic_info.ProjektInfoExtractor.extrahiere_info`, `backend.converter.process_repo_notebooks`, `backend.getRepo.GitRepository`, `backend.main.gemini_payload`, and `backend.main.update_status`. Called by `backend.main`, `frontend.frontend`.
...
*   **Usage:** This function calls no other functions. Called by `backend.main.notebook_workflow`.
...
*   **Instantiation:** `backend.main.main_workflow`
*   **Dependencies:** `backend.relationship_analyzer.CallResolverVisitor`, `backend.relationship_analyzer.path_to_module`, `ast`, `os`, `logging`, `collections.defaultdict`
*   **Constructor:**
...
*   **Methods:**
    *   **`analyze`**
        ...
        *   **Usage:** Calls `_find_py_files`, `_collect_definitions`, and `_resolve_calls`. Called by `backend.main.main_workflow`.
    *   **`get_raw_relationships`**
        ...
        *   **Usage:** This method does not explicitly call other methods, classes, or functions within its source code. Called by `backend.main.main_workflow`.
    *   **`_find_py_files`**
        ...
        *   **Usage:** Calls os.walk and os.path.join. Called by `analyze`.
    *   **`_collect_definitions`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `path_to_module`, `_get_parent`, `ast.parse`, `ast.walk`, `ast.FunctionDef`, `ast.ClassDef`, and `logging.error`. Called by `analyze`.
    *   **`_get_parent`**
        ...
        *   **Usage:** Calls ast.walk and ast.iter_child_nodes. Called by `_collect_definitions`.
    *   **`_resolve_calls`**
        ...
        *   *Returns:*
        *   **Usage:** Calls CallResolverVisitor and logging.error. Called by `analyze`.
...
*   **Methods:**
    *   **`visit_ClassDef`**
        ...
        *   *Parameters:*
            - **node** (`ast.ClassDef`): The AST node representing a class definition.
        *   *Returns:*
        *   **Usage:** Calls the generic visitor method to continue traversing the AST. Called by the AST visitor framework when a ClassDef node is encountered.
    *   **`visit_FunctionDef`**
        *   *Signature:* `def visit_FunctionDef(self, node: ast.FunctionDef)`
        ...
        *   *Parameters:*
            - **node** (`ast.FunctionDef`): The AST node representing a function definition.
        *   *Returns:*
        *   **Usage:** Calls the generic visitor method to continue traversing the AST. Called by the AST visitor framework when a FunctionDef node is encountered.
    *   **`visit_Call`**
        *   *Signature:* `def visit_Call(self, node: ast.Call)`
        ...
        *   *Parameters:*
            - **node** (`ast.Call`): The AST node representing a function or method call.
        *   *Returns:*
        *   **Usage:** Calls `_resolve_call_qname` to determine the callee's qualified name and the generic visitor method to continue traversing the AST. Called by the AST visitor framework when a Call node is encountered.
    *   **`visit_Import`**
        *   *Signature:* `def visit_Import(self, node: ast.Import)`
        ...
        *   *Parameters:*
            - **node** (`ast.Import`): The AST node representing an import statement.
        *   *Returns:*
        *   **Usage:** Calls the generic visitor method to continue traversing the AST. Called by the AST visitor framework when an Import node is encountered.
    *   **`visit_ImportFrom`**
        *   *Signature:* `def visit_ImportFrom(self, node: ast.ImportFrom)`
        ...
        *   *Parameters:*
            - **node** (`ast.ImportFrom`): The AST node representing a 'from ... import ...' statement.
        *   *Returns:*
        *   **Usage:** Calls the generic visitor method to continue traversing the AST. Called by the AST visitor framework when an ImportFrom node is encountered.
    *   **`visit_Assign`**
        ...
        *   *Returns:*
        *   **Usage:** Calls the generic visitor method to continue traversing the AST. Called by the AST visitor framework when an Assign node is encountered.
    *   **`_resolve_call_qname`**
        ...
        *   **Usage:** This method does not explicitly call other methods within its own source code. Called by `visit_Call` to resolve the qualified name of a function or method invocation.
...
*   **Returns:**
    - **module_path** (`str`): The converted Python module path string.
*   **Usage:** This function calls no other functions. Called by `backend.relationship_analyzer.CallResolverVisitor.__init__`, `backend.relationship_analyzer.ProjectAnalyzer._collect_definitions`.
...
*   **Usage:** This function calls no other functions. Called by `database.db.update_gemini_key`, `database.db.update_gpt_key`, `database.db.update_opensrc_key`.
...
*   **Usage:** This function calls no other functions. Called by `database.db.get_decrypted_api_keys`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend`.
...
*   **Usage:** This function calls no other functions. Called by no other functions.
...
*   **Usage:** Calls `database.db.encrypt_text`. Called by `frontend.frontend`, `frontend.frontend.save_gemini_cb`.
...
*   **Usage:** Calls `database.db.encrypt_text`. Called by `frontend.frontend`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend`, `frontend.frontend.save_ollama_cb`.
...
*   **Usage:** Calls `database.db.decrypt_text`. Called by `frontend.frontend`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend`, `frontend.frontend.handle_delete_chat`, `frontend.frontend.load_data_from_db`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend.load_data_from_db`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend.handle_feedback_change`.
...
*   **Returns:**
    - **modified_count** (`int`): The number of documents that were modified by the update operation.
*   **Usage:** This function calls no other functions. Called by `frontend.frontend.render_exchange`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend.handle_delete_exchange`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend.handle_delete_chat`.
...
*   **Parameters:**
*   **Returns:**
*   **Usage:** Calls `database.db.update_gemini_key`. Called by no other functions.
...
*   **Parameters:**
*   **Returns:**
*   **Usage:** Calls `database.db.update_ollama_url`. Called by no other functions.
...
*   **Returns:**
*   **Usage:** This function calls `database.db.fetch_chats_by_user`, `database.db.fetch_exchanges_by_user`, and `database.db.insert_chat`. Called by `frontend.frontend`.
...
*   **Returns:**
*   **Usage:** Calls `database.db.update_exchange_feedback`. Called by `frontend.frontend.render_exchange`.
...
*   **Returns:**
*   **Usage:** Calls `database.db.delete_exchange_by_id`. Called by `frontend.frontend.render_exchange`.
...
*   **Parameters:**
    ...
    - **chat_name** (`str`): The name of the chat to be deleted.
*   **Returns:**
*   **Usage:** Calls `database.db.delete_full_chat` and `database.db.insert_chat`. Called by `frontend.frontend`.
...
*   **Usage:** This function calls no other functions. Called by `frontend.frontend.render_text_with_mermaid`.
...
*   **Returns:**
*   **Usage:** Calls `frontend.frontend.stream_text_generator`. Called by `frontend.frontend`, `frontend.frontend.render_exchange`.
...
*   **Returns:**
*   **Usage:** Calls `database.db.update_exchange_feedback_message`, `frontend.frontend.handle_delete_exchange`, `frontend.frontend.handle_feedback_change`, and `frontend.frontend.render_text_with_mermaid`. Called by `frontend.frontend`.
...
*   **Instantiation:** *Analysis data not available for this component.*
*   **Dependencies:** `List`, `ParameterDescription`, `ReturnDescription`, `UsageContext`
*   **Constructor:**
...
*   **Instantiation:** `backend.HelperLLM.main_orchestrator`, `backend.main.main_workflow`
*   **Dependencies:** `pydantic.BaseModel`, `typing.Literal`, `typing.List`
*   **Constructor:**
