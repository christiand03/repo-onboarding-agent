...
## 1. Project Overview
- **Description:** This project is an automated documentation agent designed to analyze Git repositories. It clones a given repository, builds an Abstract Syntax Tree (AST) to understand the code structure, and utilizes a dual Large Language Model (LLM) architecture to perform detailed analysis of functions and classes. The final output is a comprehensive, human-readable documentation report. The system features a web-based user interface built with Streamlit for user interaction and presenting the results.
- **Key Features:**
  - Git Repository Cloning & File Analysis
  - Abstract Syntax Tree (AST) Generation for Code Structure Mapping
  - Dual-LLM Architecture for Granular Code Analysis & Synthesis
  - Interactive Streamlit Web Frontend
  - Support for Multiple LLM Providers (Gemini, OpenAI, Ollama)
- **Tech Stack:** Langchain, Streamlit, Pydantic, GitPython, Networkx, TOON Format
...
*   **Repository Structure:**
    ...
    graph LR
        Project --> backend;
        Project --> database;
        Project -- main entry --> frontend;
        Project --> schemas;

        backend --> backend_files["AST_Schema.py<br/>File_Dependency.py<br/>HelperLLM.py<br/>MainLLM.py<br/>basic_info.py<br/>callgraph.py<br/>converter.py<br/>getRepo.py<br/>main.py<br/>relationship_analyzer.py"];
        database --> db_files["db.py"];
...
## 2. Installation
### Dependencies
As this project includes a `requirements.txt` file, you can install all dependencies with the command: `pip install -r requirements.txt`.
...
```
### Setup Guide
1.  Clone the repository: `git clone https://github.com/your-username/repo-onboarding-agent.git`
2.  Navigate into the project directory: `cd repo-onboarding-agent`
3.  Create a `.env` file from the example: `cp .env.example .env`
4.  Edit the `.env` file to add your required API keys (e.g., `GEMINI_API_KEY`, `OPENAI_API_KEY`).
...
This application is designed to streamline the process of understanding and documenting codebases.

**Use Cases:**
- **Automated Documentation:** Generate detailed documentation for any public Python Git repository by simply providing its URL.
- **Developer Onboarding:** Help new developers quickly understand the architecture, dependencies, and key components of a complex codebase.
- **Code Auditing:** Provide a high-level overview of a project's structure, functions, classes, and their relationships for code reviews or refactoring.
- **Jupyter Notebook Analysis:** Automatically document Jupyter Notebooks, including their markdown cells, code, outputs, and embedded images.

**Primary Command:**
The application is controlled via a web interface. To start the service, run the following command from the project's root directory:
```bash
streamlit run frontend/frontend.py
```
After starting the application, open your web browser to the provided local URL. You can then input a Git repository URL and select your desired LLM models to begin the analysis.
...
*   **Methods:**
    ...
    *   **`extrahiere_info`**
        ...
        *   **Usage:**
            ...
            - **Called By:** This method is not explicitly called by other methods within the provided context, suggesting it is a primary entry point for external usage.
...
*   **Methods:**
    *   **`blob`**
        ...
        *   **Usage:**
            ...
            - **Called By:** This method is not explicitly called by other components.
    *   **`content`**
...
*   **Methods:**
    ...
    *   **`__enter__`**
        ...
        *   **Usage:**
            ...
            - **Called By:** This method is not explicitly called by other methods in the provided context.
    *   **`__exit__`**
        *   *Signature:* `def __exit__(self: GitRepository, exc_type: type | None, exc_val: Exception | None, exc_tb: TracebackType | None)`
...
*   **Methods:**
    ...
    *   **`_collect_definitions`**
        *   *Signature:* `def _collect_definitions(filepath: string)`
    ...
    *   **`_resolve_calls`**
        *   *Signature:* `def _resolve_calls(filepath: string)`
...
*   **Methods:**
    ...
    *   **`_resolve_call_qname`**
        ...
        *   **Usage:**
            ...
            - **Called By:** This method is called by `visit_Call` to resolve the qualified name of a function or method invocation.
