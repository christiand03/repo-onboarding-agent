...
## 1. Project Overview
- **Description:** The Repo Onboarding Agent is an automated analysis tool designed to help developers quickly understand codebase structures. It utilizes a Streamlit frontend to accept repository URLs, clones them, analyzes Python files using AST parsing, and employs Large Language Models (LLMs) like Gemini and OpenAI to generate detailed documentation for classes and functions.
- **Key Features:**
  - **Automated Code Analysis**: AST-based parsing of Python files to identify functions, classes, and dependencies.
  - **LLM-Powered Documentation**: Uses Helper and Main LLMs to generate human-readable descriptions for code components.
  - **Repository Visualization**: Generates dependency graphs and call graphs.
  - **Interactive Frontend**: Streamlit-based UI for submitting repos, viewing results, and interacting with the analysis via chat.
  - **Notebook Support**: Converts and analyzes Jupyter Notebooks (`.ipynb`) alongside standard Python files.
- **Tech Stack:** Python, Streamlit, LangChain, MongoDB (PyMongo), GitPython, NetworkX, Google Gemini, OpenAI.
...
### Setup Guide
To install the required dependencies, run the following command in the repository root:
`pip install -r requirements.txt`

### Quick Startup
1.  Configure environment variables (API keys for Gemini/OpenAI, Database URI).
2.  Start the Streamlit application:
    `streamlit run frontend/frontend.py`

## 3. Use Cases & Commands
Based on the repository analysis, the following use cases and commands are identified:

1.  **Repository Analysis**: Users can input a GitHub repository URL via the frontend. The system clones the repo, extracts file information, and analyzes code structure using AST parsing and relationship analyzers.
2.  **Documentation Generation**: The tool automatically generates comprehensive documentation for functions and classes using LLMs (Helper LLM for components, Main LLM for final reports).
3.  **Notebook Analysis**: The system supports `.ipynb` files, converting them to XML and processing them to extract code and markdown for analysis.
4.  **Interactive Q&A**: Users can chat with the "Agent" about the analyzed codebase, with exchanges stored in a MongoDB database.
5.  **Visualization**: Users can view dependency graphs and call graphs generated from the codebase analysis.
...
*   **Methods:**
    *   **`visit_Import`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `self.generic_visit` to continue AST traversal. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.Import` node is encountered.
    *   **`visit_ImportFrom`**
        ...
        *   *Returns:*
        *   **Usage:** Calls `self.generic_visit` to continue AST traversal. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.ImportFrom` node is encountered.
    *   **`visit_ClassDef`**
        ...
        *   *Returns:*
        *   **Usage:** This method calls `ast.get_docstring`, `ast.get_source_segment`, and `self.generic_visit`. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.ClassDef` node is encountered.
    *   **`visit_FunctionDef`**
        ...
        *   *Returns:*
        *   **Usage:** This method calls `ast.get_docstring`, `ast.get_source_segment`, and `self.generic_visit`. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.FunctionDef` node is encountered, and also by `visit_AsyncFunctionDef`.
    *   **`visit_AsyncFunctionDef`**
        ...
        *   *Returns:*
        *   **Usage:** This method calls `self.visit_FunctionDef`. Called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.AsyncFunctionDef` node is encountered.
...
*   **Methods:**
    *   **`_resolve_module_name`**
        *   *Signature:* `def _resolve_module_name(node)`
...
*   **Usage:** This function calls backend.File_Dependency.FileDependencyGraph. Called by no other functions.
...
*   **Usage:** This function calls backend.callgraph.CallGraph. Called by no other functions.
