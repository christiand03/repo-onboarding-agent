...
## 1. Project Overview
- **Description:** A comprehensive 'Repo Onboarding Agent' designed to automate the documentation process for Git repositories. It leverages Large Language Models (LLMs) to analyze code structure, identify functions and classes, resolve dependencies, and generate detailed, human-readable reports. The system also includes a user-friendly frontend for interaction and manages user API keys securely.
- **Key Features:**
  - Automated Git Repository Cloning and File Processing.
  - Deep Code Analysis via Abstract Syntax Tree (AST) Generation.
  - LLM-Powered Extraction of Function, Class, and Method Documentation.
  - Automated Report Generation in Markdown format.
  - Secure User and API Key Management.
  - Interactive Streamlit-based Web Frontend.
- **Tech Stack:** Python, Streamlit, LangChain, Google Gemini API, OpenAI API, Ollama, GitPython, PyMongo, Pydantic, NetworkX, Matplotlib

*   **Repository Structure:**
    ```mermaid
    graph LR
      root["root"]
      root --> dir___env_example
      dir___env_example["_env_example"]
      root --> _gitignore
      _gitignore[".gitignore"]
      root --> SystemPrompts
      SystemPrompts["SystemPrompts<br/>SystemPromptClassHelperLLM.txt<br/>SystemPromptFunctionHelperLLM.txt<br/>SystemPromptHelperLLM.txt<br/>SystemPromptMainLLM.txt<br/>SystemPromptMainLLMToon.txt<br/>SystemPromptNotebookLLM.txt"]
      root --> analysis_output_json
      analysis_output_json["analysis_output.json"]
      root --> backend
      backend["backend<br/>AST_Schema.py<br/>File_Dependency.py<br/>HelperLLM.py<br/>MainLLM.py<br/>__init__.py<br/>basic_info.py<br/>callgraph.py<br/>converter.py<br/>getRepo.py<br/>main.py<br/>relationship_analyzer.py<br/>scads_key_test.py"]
      root --> database
      database["database<br/>db.py"]
      root --> frontend
      frontend["frontend<br/>__init__.py<br/>frontend.py"]
      frontend --> _streamlit
      _streamlit[".streamlit<br/>config.toml"]
      frontend --> gifs
      gifs["gifs<br/>4j.gif"]
      root --> notizen
      notizen["notizen<br/>Report Agenda.txt<br/>Zwischenpraesentation Agenda.txt<br/>doc_bestandteile.md<br/>notizen.md<br/>paul_notizen.md<br/>praesentation_notizen.md<br/>technische_notizen.md"]
      notizen --> grafiken
      grafiken["grafiken"]
      grafiken --> 1
      1["1<br/>File_Dependency_Graph_Repo.dot<br/>global_callgraph.png<br/>global_graph.png<br/>global_graph_2.png<br/>repo.dot"]
      grafiken --> 2
      ...
      grafiken --> Repo_onboarding
      Repo_onboarding["Repo-onboarding<br/>AST.dot<br/>Frontend.dot<br/>HelperLLM.dot<br/>HelperLLM.png<br/>MainLLM.dot<br/>agent.dot<br/>basic_info.dot<br/>callgraph.dot<br/>getRepo.dot<br/>graph_AST.png<br/>graph_AST2.png<br/>graph_AST3.png<br/>main.dot<br/>tools.dot<br/>types.dot"]
      root --> output_json
...
**Use Cases:**
- Automated documentation generation for any public GitHub repository.
- LLM-driven deep code analysis, extracting detailed descriptions, parameters, returns, and usage contexts for Python functions, classes, and methods.
- Creation of structured, human-readable Markdown reports that integrate architectural insights and code-level documentation.
- Secure management of user API keys for various Large Language Model (LLM) services, ensuring confidential access.
- Interactive exploration and chat-based analysis of repository documentation through a Streamlit-powered web frontend.

**Commands:**
To start the application, navigate to the project root and run the Streamlit application:
```bash
streamlit run frontend/frontend.py
```
This will launch the interactive web interface where you can provide a GitHub repository URL and select LLM models for analysis and report generation.
...
*   **Methods:**
    *   **`visit_Import`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls `self.generic_visit` to continue AST traversal. This method is called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.Import` node is encountered.
    *   **`visit_ImportFrom`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls `self.generic_visit` to continue AST traversal. This method is called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.ImportFrom` node is encountered.
    *   **`visit_ClassDef`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls `ast.get_docstring`, `ast.get_source_segment`, and `self.generic_visit`. This method is called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.ClassDef` node is encountered.
    *   **`visit_FunctionDef`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls `ast.get_docstring`, `ast.get_source_segment`, and `self.generic_visit`. This method is called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.FunctionDef` node is encountered, and also by `visit_AsyncFunctionDef`.
    *   **`visit_AsyncFunctionDef`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls `self.visit_FunctionDef`. This method is called by the `ast.NodeVisitor`'s dispatch mechanism when an `ast.AsyncFunctionDef` node is encountered.
...
*   **Methods:**
    ...
    *   **`module_file_exists`**
        ...
        *   *Parameters:*
            - **rel_base** (`str`): No description provided.
            - **name** (`str`): No description provided.
        *   *Returns:* No returns.
        *   **Usage:** No usage context available.
    *   **`init_exports_symbol`**
        *   *Signature:* `def init_exports_symbol(rel_base, symbol)`
        *   *Description:* Analysis data not available for this component.
        *   *Parameters:*
            - **rel_base** (`str`): No description provided.
            - **symbol** (`str`): No description provided.
        *   *Returns:* No returns.
        *   **Usage:** No usage context available.
    *   **`visit_Import`**
...
*   **Parameters:**
    - **graph** (`networkx.DiGraph`): The NetworkX directed graph to be converted into a DOT file.
...
*   **Methods:**
    *   **`blob`**
        ...
        *   **Usage:** This method does not explicitly call other methods or functions. This method is not explicitly called by other components.
    *   **`content`**
...
*   **Methods:**
    *   **`get_all_files`**
        ...
        *   **Usage:** This method calls `backend.getRepo.RepoFile` to create `RepoFile` instances. This method is not explicitly called by other methods in the provided context.
    *   **`close`**
        ...
        *   *Parameters:*
            - **self** (`GitRepository`): The instance of the GitRepository.
        *   *Returns:* None
        *   **Usage:** This method does not explicitly call other functions or methods. This method is not explicitly called by other methods in the provided context.
    *   **`__enter__`**
...
*   **Usage:** This function calls backend.MainLLM.MainLLM, backend.MainLLM.MainLLM.call_llm, backend.basic_info.ProjektInfoExtractor, backend.basic_info.ProjektInfoExtractor.extrahiere_info, backend.converter.process_repo_notebooks, backend.getRepo.GitRepository, backend.main.gemini_payload, and backend.main.update_status. This function is not called by any other functions.
...
*   **Methods:**
    *   **`analyze`**
        ...
        *   **Usage:** This method calls _find_py_files, _collect_definitions, and _resolve_calls. This method is not explicitly called by other methods within the provided method_context.
    *   **`get_raw_relationships`**
        ...
        *   **Usage:** This method does not explicitly call other methods, classes, or functions within its source code. This method is not explicitly called by other methods within the provided method_context.
    *   **`_find_py_files`**
        ...
        *   **Usage:** This method calls os.walk and os.path.join. This method is called by analyze.
    *   **`_collect_definitions`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls path_to_module, _get_parent, ast.parse, ast.walk, ast.FunctionDef, ast.ClassDef, and logging.error. This method is called by analyze.
    *   **`_get_parent`**
        ...
        *   **Usage:** This method calls ast.walk and ast.iter_child_nodes. This method is called by _collect_definitions.
    *   **`_resolve_calls`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls CallResolverVisitor and logging.error. This method is called by analyze.
...
*   **Methods:**
    *   **`visit_ClassDef`**
        ...
        *   *Parameters:*
            - **node** (`ast.ClassDef`): The AST node representing a class definition.
        *   *Returns:* None
        *   **Usage:** This method calls the generic visitor method to continue traversing the AST. This method is called by the AST visitor framework when a ClassDef node is encountered.
    *   **`visit_FunctionDef`**
        *   *Signature:* `def visit_FunctionDef(self, node)`
        ...
        *   *Parameters:*
            - **node** (`ast.FunctionDef`): The AST node representing a function definition.
        *   *Returns:* None
        *   **Usage:** This method calls the generic visitor method to continue traversing the AST. This method is called by the AST visitor framework when a FunctionDef node is encountered.
    *   **`visit_Call`**
        *   *Signature:* `def visit_Call(self, node)`
        ...
        *   *Parameters:*
            - **node** (`ast.Call`): The AST node representing a function or method call.
        *   *Returns:* None
        *   **Usage:** This method calls `_resolve_call_qname` to determine the callee's qualified name and the generic visitor method to continue traversing the AST. This method is called by the AST visitor framework when a Call node is encountered.
    *   **`visit_Import`**
        *   *Signature:* `def visit_Import(self, node)`
        ...
        *   *Parameters:*
            - **node** (`ast.Import`): The AST node representing an import statement.
        *   *Returns:* None
        *   **Usage:** This method calls the generic visitor method to continue traversing the AST. This method is called by the AST visitor framework when an Import node is encountered.
    *   **`visit_ImportFrom`**
        *   *Signature:* `def visit_ImportFrom(self, node)`
        ...
        *   *Parameters:*
            - **node** (`ast.ImportFrom`): The AST node representing a 'from ... import ...' statement.
        *   *Returns:* None
        *   **Usage:** This method calls the generic visitor method to continue traversing the AST. This method is called by the AST visitor framework when an ImportFrom node is encountered.
    *   **`visit_Assign`**
        ...
        *   *Returns:* None
        *   **Usage:** This method calls the generic visitor method to continue traversing the AST. This method is called by the AST visitor framework when an Assign node is encountered.
    *   **`_resolve_call_qname`**
        ...
        *   **Usage:** This method does not explicitly call other methods within its own source code. This method is called by `visit_Call` to resolve the qualified name of a function or method invocation.
